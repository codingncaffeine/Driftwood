...
{
    /// <summary>
    /// Resolves a Java model texture identifier onto the fixed runtime array. Models may spell a
    /// namespace, omit <c>textures/</c>, and omit <c>.png</c>; the mapping table remains the single
    /// authority for which foreign picture corresponds to which Driftwood layer.
    /// </summary>
    public static bool TryLayerForResource(string resource, out ushort layer)
    {
        layer = 0;
        if (string.IsNullOrWhiteSpace(resource) || resource[0] == '#') return false;

        resource = resource.Replace('\\', '/').Trim();
        var colon = resource.IndexOf(':');
        if (colon >= 0) resource = resource[(colon + 1)..];
        resource = resource.TrimStart('/');
        if (!resource.StartsWith("textures/", StringComparison.OrdinalIgnoreCase))
            resource = $"textures/{resource}";
        if (!resource.EndsWith(".png", StringComparison.OrdinalIgnoreCase)) resource += ".png";

        for (var i = 0; i < Layers.Length; i++)
        {
            var mapping = Layers[i];
            if (string.Equals(mapping.PackPath, resource, StringComparison.OrdinalIgnoreCase)
                || mapping.PackPathAlt.Length > 0 && string.Equals(
                    mapping.PackPathAlt, resource, StringComparison.OrdinalIgnoreCase))
            {
                layer = checked((ushort)i);
                return true;
            }

            // Pre-flattening names are generated by the same layout table used by TexturePack.
            if (mapping.PackPath.Length > 0 && PackLayouts.Legacy(mapping.PackPath).Any(candidate =>
                    string.Equals(candidate, resource, StringComparison.OrdinalIgnoreCase)))
            {
                layer = checked((ushort)i);
                return true;
            }
